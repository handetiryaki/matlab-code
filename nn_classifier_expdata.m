function [ z ] = nn_classifier_expdata(hastfidfweighting,feature_size,feature_selection_method,dataset_type)
...
micro_matrix=zeros(1,validation_count);
macro_matrix=zeros(1,validation_count);
num_true_classifications_matrix=zeros(1,validation_count);
num_false_classifications_matrix=zeros(1,validation_count);

%average score calculations begin
for counter=1:validation_count

target=zeros(length(topic_matrix),length(dataset_class_idx));
for i=1:length(topic_matrix)
    target(i,topic_matrix(i,1))=1;
end
t=target';
p=train_matrix';

net=newff(minmax(p),[20  20 length(dataset_class_idx)],{'tansig' 'tansig' 'purelin'},'traingdx');
net.trainParam.epochs=1000;
net.trainParam.goal=0.01;
net=train(net,p,t);

predicted_labels=sim(net,test_matrix');
[~,ind] = max(predicted_labels(:,:));
...
micro_matrix(1,counter)=micro_F1;
macro_matrix(1,counter)=macro_F1;
num_true_classifications_matrix(1,counter)=num_true_classifications;
num_false_classifications_matrix(1,counter)=num_false_classifications;


end

%average score calculations end

micro_F1=mean(micro_matrix);
macro_F1=mean(macro_matrix);
num_true_classifications=mean(num_true_classifications_matrix);
num_false_classifications=mean(num_false_classifications_matrix);
...
if (hastfidfweighting==1)
    result_fpath=strcat('D:\phddocuments\feature_selection_with_experimentaldata\',resfoldername,'\nnres_with_tfidf.txt');
else
    result_fpath=strcat('D:\phddocuments\feature_selection_with_experimentaldata\',resfoldername,'\nnres_without_tfidf.txt');
end

fresid = fopen(result_fpath, 'a');
fprintf(fresid,'%d %d %d %6.2f %6.2f %6.3f %6.3f\n',feature_selection_method,length(term_feature),feature_size,num_true_classifications,num_false_classifications,micro_F1,macro_F1);
fclose(fresid);
